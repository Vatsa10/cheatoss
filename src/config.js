...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        // Windows: %APPDATA%\cheatoss-config
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'cheatoss-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/cheatoss-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'cheatoss-config');
    } else {
        // Linux and others: ~/.config/cheatoss-config
        configDir = path.join(os.homedir(), '.config', 'cheatoss-config');
    }
